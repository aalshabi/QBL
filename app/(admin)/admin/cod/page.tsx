...
export default async function AdminCodPage() {
  ...
  const { custody, settlements, expenses, summary } =
    await getAdminDataSource().getCod();

  const kpis = [
    {
      label: "COD في عهدة المناديب",
      value: formatSar(summary.codWithCouriers),
      icon: HandCoins,
      tone: "text-amber-600",
    },
    {
      label: "مستحق للعملاء",
      value: formatSar(summary.dueToClients),
      icon: Banknote,
      tone: "text-sky-600",
    },
    {
      label: "أجور التوصيل (مسلَّم)",
      value: formatSar(summary.deliveryFeesSum),
      icon: Receipt,
      tone: "text-[#00A7B6]",
    },
    {
      label: "صافي الربح",
      value: formatSarExact(summary.netProfit),
      icon: Landmark,
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="rounded border border-amber-300 bg-amber-50 p-3 text-sm">
        الأرقام التشغيلية تحتاج مطابقة المحاسب ودفترة. أجور المرتجعات والضريبة
        والربح غير متاحة حتى اعتماد العقود والفواتير والتكاليف؛ لا تفترض هذه
        الشاشة أجرة ثابتة للمرتجع.
      </p>
          ...
          <Card key={kpi.label}>
            <CardContent className="flex items-center justify-between p-5">
              <div>
                ...
                <p className="mt-1 text-xl font-bold text-[#0D1B3A]">
                  {kpi.value}
                </p>
              </div>
      ...
      <Tabs defaultValue="custody">
        <TabsList>
          <TabsTrigger value="custody">
            عُهدات المناديب ({formatNumber(custody.length)})
          </TabsTrigger>
          <TabsTrigger value="settlements">
            كشوفات العملاء ({formatNumber(settlements.length)})
          </TabsTrigger>
          <TabsTrigger value="expenses">
            المصروفات ({formatNumber(expenses.length)})
          </TabsTrigger>
                  ...
                  {custody.map((row) => (
                    <TableRow key={row.courierId}>
                      ...
                      <TableCell className="font-semibold text-amber-700">
                        {formatSar(row.totalCod)}
                      </TableCell>
                      <TableCell>{formatSar(row.totalFees)}</TableCell>
                      <TableCell className="font-semibold">
                        {formatSar(row.totalCod - row.totalFees)}
                      </TableCell>
                    </TableRow>
                  ))}
                  {custody.length === 0 && (
                  ...
                  {settlements.map((row) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell className="font-semibold text-emerald-700">
                        {formatSarExact(row.netToClient)}
                      </TableCell>
                      <TableCell className="whitespace-nowrap text-xs text-slate-500">
                        {formatDateTime(
                          row.deliveredAt ?? row.exportedAt ?? row.sortedAt,
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                <Row
                  label="مجموع COD المسلَّم"
                  value={formatSar(summary.deliveredCodSum)}
                />
                <Row
                  label="أجور التوصيل (مسلَّم)"
                  value={formatSar(summary.deliveryFeesSum)}
                ...
                <Row
                  label="أجور الرواجع"
                  value={formatSarExact(summary.returnedFeesSum)}
                />
                <Row
                  label="الضريبة المضافة (15٪)"
                  value={formatSarExact(summary.vatSum)}
                />
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                <Row
                  label="مصروفات المناديب"
                  value={formatSar(summary.expensesByType.DRIVER)}
                />
                <Row
                  label="مصروفات الفروع"
                  value={formatSar(summary.expensesByType.HUB)}
                />
                <Row
                  label="مصروفات الشركاء"
                  value={formatSar(summary.expensesByType.PARTNER)}
                />
                <Row
                  label="مصروفات أخرى"
                  value={formatSar(summary.expensesByType.OTHER)}
                />
                <div className="border-t pt-2">
                  <Row
                    label="إجمالي المصروفات"
                    value={formatSar(summary.expensesSum)}
                    strong
                  />
                  <Row
                    label="صافي الربح"
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between py-1">
      ...
      <span
        className={`${strong ? "font-bold" : "font-medium"} ${accent ? "text-emerald-700" : "text-[#0D1B3A]"}`}
      >
        {value}
      </span>
    </div>
